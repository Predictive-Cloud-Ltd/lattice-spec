...
const frag = (over = {}) => ({ topologyVersion: "0.3.0", scope: "fragment", producer: { name: "p", provider: "x", authority: 0 }, nodes: [], ...over });
...
test("incompatible topologyVersion majors throw", () => {
  const a = frag({ topologyVersion: "0.3.0", nodes: [{ id: "N", kind: "inverter" }] });
...
test("deviceTypes are carried into the merged site so node deviceType resolves", () => {
  const fragDoc = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "gw", provider: "gw", authority: 0 }, docVersion: 1,
...
test("deviceTypes union dedups by key across fragments (higher authority wins)", () => {
  const a = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "gw", provider: "gw", authority: 0 }, docVersion: 1,
    deviceTypes: [{ key: "ge-aio", capabilities: [{ capability: "battery.soc", read: {} }] }],
    nodes: [{ id: "INV", kind: "inverter", deviceType: "ge-aio" }] };
  const b = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "i", provider: "installer", authority: 50 }, docVersion: 1,
...
test("site.id survives from a low-authority input when the top input omits id", () => {
  const disc = { topologyVersion: "0.3.0", scope: "fragment", id: "site:home", producer: { name: "gw", provider: "gw", authority: 0 }, docVersion: 1, nodes: [{ id: "INV", kind: "inverter" }] };
  const over = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "installer", provider: "installer", authority: 50 }, docVersion: 1, nodes: [{ id: "INV", kind: "gateway" }] };
  const { site } = merge([disc, over]);
...
test("site.id: higher-authority input wins when both inputs set id", () => {
  const low = { topologyVersion: "0.3.0", scope: "fragment", id: "site:discovered", producer: { name: "gw", provider: "gw", authority: 0 }, docVersion: 1, nodes: [{ id: "N", kind: "inverter" }] };
  const high = { topologyVersion: "0.3.0", scope: "fragment", id: "site:authoritative", producer: { name: "i", provider: "installer", authority: 50 }, docVersion: 1, nodes: [{ id: "N", kind: "inverter" }] };
...
test("access paths at equal preference sort by id codepoint order (not locale)", () => {
  const frag = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "gw", provider: "gw", authority: 0 }, docVersion: 1,
...
test("tombstone barrier: re-add above a removal does not resurrect sub-barrier discovery", () => {
  const disc = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "gw", provider: "gw", authority: 0 }, docVersion: 1,
  ...
  const remove = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "installer", provider: "installer", authority: 50 }, docVersion: 1, nodes: [{ id: "N", removed: true }] };
  const readd = { topologyVersion: "0.3.0", scope: "fragment", producer: { name: "admin", provider: "admin", authority: 100 }, docVersion: 1, nodes: [{ id: "N", kind: "gateway" }] };
  const { site } = merge([disc, remove, readd]);
  // node survives (re-added above the tombstone) but carries ONLY the above-barrier admin data
  assert.equal(site.nodes.length, 1);
  assert.deepEqual(site.nodes[0], { id: "N", kind: "gateway" });
});
