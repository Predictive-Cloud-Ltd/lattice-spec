...
**Resolution for** [#3 coupled / mode / read-modify-write control](https://github.com/Predictive-Cloud-Ltd/lattice-spec/issues/3). **Implemented (PR-C):** the `0.1.0/` schema now carries `shape` (`setpoint`/`switch`/`schedule`), `controlGroup` (coupled atomic writes), `tier` (L1/L2), and a `readModifyWrite` binding flag, with conformance + a worked example. **Ownership/arbitration stays runtime-only** by decision (a claim is live, time-bounded state; the resolver derives the claimed subtree from `contains`/`aggregate` relationships — no schema construct; see §6). The remaining open piece is the **scheme vocabulary content** (the agreed `battery.mode` value set) — a domain-owner follow-up; the schema treats it as an open string.
...
2a. The **scheme vocabulary** itself — **DECIDED to keep OPEN (PR-C):** `battery.mode` is an open string with a documented starter set (`self_use`, `max_self_use`, `force_charge`, `export`, `idle`, `backup`, `eco`) + `x-<vendor>:` extensions, so it extends without a schema change. **Still a domain-owner follow-up:** confirming/finalising that set (and whether schemes carry intensity/parameters — "max self-use" vs "self-use") with the maintainers.
