...
| `GE_RATE_FULL` (HR313/314: read raw×rated/100 trunc; write watts→% round-half-up, clamp 100) | read `{ kind: ratio, num: { ref: "rated_power" }, den: 100 }`; control `{ kind: ratio, num: { ref: "rated_power" }, den: 100, round: half_up, max: 100, onRefUnavailable: max }`, executed via `fromEng` (same kind/num/den as the read; `round: half_up` overrides the read's default `trunc`, so writes round half-up) |
